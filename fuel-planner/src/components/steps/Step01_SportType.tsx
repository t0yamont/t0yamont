...
export default function Step01_SportType({ state, onChange, onNext }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-3rem)] px-4 max-w-md md:max-w-3xl mx-auto w-full">
      <motion.div
      ...
      >
        <div className="text-center space-y-2 md:space-y-3">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>
        ...
        <div className="grid gap-3 md:gap-5 md:grid-cols-3">
          {SPORT_CARDS.map((card, i) => (
            <motion.button
              ...
              className={`group relative flex md:flex-col items-center gap-4 p-5 md:p-7 md:text-center rounded-2xl border transition-colors text-left ${
                state.sport === card.id
            ...
            >
              <div className="text-4xl md:text-5xl leading-none">{card.icon}{card.subIcon}</div>
              <div className="flex-1">
                <div className="text-white font-bold text-lg md:text-xl" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>
                  {card.label}
                </div>
                <div className="text-slate-400 text-sm">{card.desc}</div>
              </div>
              {state.sport === card.id && (
                <div className="w-2 h-2 rounded-full bg-cyan-400 shrink-0 md:absolute md:top-3 md:right-3" />
              )}
            </motion.button>
          ))}
        </div>
